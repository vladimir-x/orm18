...
Простая реализация модели объектно-реляционного отображения данных при работе с PostgreSQL для Java SE приложений.
Описание модели производится в классах - сущнотях по аналоги с JPA. Для работы с БД генерируются соответвующие SQL инструкции.
В связи с отсутсвием нормального пула потоков и менеджера транзакций - использование в реальных проектах не рекомендуется (лучше обратите внимание на поплярные промышленные фреймворки).
...
* Для описания условий автоматически генерирует статических моделей классов-сущностей.
